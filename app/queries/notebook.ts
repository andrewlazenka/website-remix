...
export async function createNotebookEntry({
  ...
  word_count,
  read_time_minutes
}: Pick<NotebookEntry, 'title' | 'content' | 'date_published' | 'is_published' | 'word_count' | 'read_time_minutes'>,
  { auth }: { auth: string }) {
...
export async function getNotebookEntryBySlug(slug: string, isAuthenticated: boolean = false) {
