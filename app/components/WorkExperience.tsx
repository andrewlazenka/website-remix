...
function WorkExperience({
  ...
  return (
    <article className="flex flex-col justify-between px-4 py-8 pl-0 md:flex-row">
      ...
      <div className="align-center mt-6 flex flex-col justify-center text-center md:mt-0 md:w-2/5 md:text-left lg:w-3/5">
        {is_published ? (
          <InternalLink to={`/journey/${slug}`}>
            <h3 className="font-medium text-ctp-sapphire">{position}</h3>
          </InternalLink>
        ) : (
          <h3 className="font-medium text-ctp-sapphire">{position}</h3>
        )}
        <p className="m-0 text-ctp-yellow">{company}</p>
        <p className="m-0 text-ctp-subtext0">{`${startDate} - ${endDate}`}</p>
      </div>
    </article>
  )
}
